function [fit_parameters,residual,p] = fitting_lam2(dataset, param_guess, p, lb, ub)
...
function val = residualsfunction(param)          
    lam2= param;    % decay rate of T cells
   % p.IC = [p.L0;p.V0;p.Th0;p.B0;p.P0;p.M0;p.A0;p.I0];
     tspan=[0 210];
    % call the solver, and convert to log10
     [time2,sol,~ ]= Humoral_response_model2(p,tspan,lam2);
     yvalstrue= sol(9,:);
     idx1 = find(time2>=30,1);
     idx2 = find(time2>=90,1);
     idx3 = find(time2>=180,1);
     idxx = [idx1,idx2,idx3];
